...
def handle_query(call):
    if call.data == "my_sub":
        ...
        msg = bot.send_message(
            ...
            "Отправьте данные следующим образом (через пробел):\n"
            "<code>КОД НАГРАДА [ЛИМИТ АКТИВАЦИЙ]</code>\n\n"
            "<b>Примеры:</b>\n"
            "• <code>FREE3 3 10</code> — даст 3 дня подписки, сработает ровно 10 раз\n"
            "• <code>SALE20 20% 50</code> — даст скидку 20%, сработает 50 раз\n"
            "• <code>VIP30 30</code> — даст 30 дней, без ограничения по количеству", 
            parse_mode="HTML"
        )
